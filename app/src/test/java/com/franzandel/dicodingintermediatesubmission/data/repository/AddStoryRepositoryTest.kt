...
class AddStoryRepositoryTest {
    ...
    @Before
    fun setUp() {
        addStoryRepository = AddStoryRepositoryImpl(addStoryRemoteSource)
    }

    @Test
    fun `when uploadImage return success`() = runTest {
        val expectedUploadImage =
            Result.Success(BaseResponse(error = false, message = "upload image success"))
        val fakeToken = "asdf"
        val fakeAddStoryRequest = getFakeAddStoryRequest()

        Mockito.`when`(addStoryRemoteSource.uploadImage(fakeToken, fakeAddStoryRequest))
            .thenReturn(expectedUploadImage)

        val actualUploadImage = addStoryRepository.uploadImage(fakeToken, fakeAddStoryRequest)
        Mockito.verify(addStoryRemoteSource).uploadImage(fakeToken, fakeAddStoryRequest)
    ...
    fun `when uploadImage return failed`() = runTest {
        ...
        val expectedUploadImage =
            Result.Error(
                fakeResponseCode,
                BaseResponse(error = true, message = "upload image failed")
            )
        val fakeToken = "asdf"
        val fakeAddStoryRequest = getFakeAddStoryRequest()
